...
- Steps:
  1. In `dashboard/views.py`, right after the DataFrame loads, add:
     # Part of beginner examples, Example 1
...
- Why it helps: Reinforces how pandas manipulates data before the template renders anything, and how easy it is to add a calculated column to a dataset before render.
...
- Steps:
  1. Copy `data/sample.csv` to `data/quarterly_revenue.csv` and adjust the headers to `quarter,total_sales`.
     Open the .CSV, delete the current data, and insert your own data (for example):
    ...
    Note: In `dashboard/static/dashboard/styles.css` you can see how the table is styled for both dark and light modes
...
- Steps:
  1. In `dashboard/views.py`, add:
     ...
     When you build the `context` dictionary later in the view, include `months` so the template can render the options. Example:
...
## Example 5: "Hello, Django" Mini Page
...
## Example 6: Intro to Template Inheritance (Base Layout)
...
## Example 7: Static Asset Swap (Custom CSS)
